    ...
    <div className="flex flex-col gap-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="font-body-md text-body-md text-on-surface-variant mt-2">{t('audit.subtitle')}</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-4 py-4 border-y border-outline-variant">
        <select value={entity} onChange={(e) => setEntity(e.target.value)} className={selectCls}>
          <option value="" className="bg-surface-container">{t('audit.all_entities')}</option>
          {['user', 'product', 'variant', 'order', 'category', 'brand'].map((e) => (
            <option key={e} value={e} className="bg-surface-container">{t('audit.entity_' + e)}</option>
          ))}
        </select>
        <select value={action} onChange={(e) => setAction(e.target.value)} className={selectCls}>
          <option value="" className="bg-surface-container">{t('audit.all_actions')}</option>
          {['login', 'logout', 'create', 'update', 'delete', 'inventory_change'].map((a) => (
            <option key={a} value={a} className="bg-surface-container">{t('audit.action_' + a)}</option>
          ))}
        </select>
        <span className="font-label-sm text-label-sm text-on-surface-variant ml-auto">{logs.length} {t('audit.entries')}</span>
      </div>
      ...
      <div className="bg-surface-container-low border border-outline-variant">
        <div className="w-full overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[960px]">
            <thead>
              <tr className="border-b border-outline-variant">
                {[t('audit.time'), t('audit.user'), t('audit.action'), t('audit.entity'), t('audit.entity_id'), t('audit.details')].map((h) => (
              ...
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-surface-container transition-colors">
                  ...
                  <td className="py-4 px-6 max-w-xs">
                    {log.old_values && (
                      <span className="block text-xs text-error truncate">{t('audit.old')}: {JSON.stringify(log.old_values)}</span>
                    )}
                    {log.new_values && (
                      <span className="block text-xs text-secondary truncate">{t('audit.new')}: {JSON.stringify(log.new_values)}</span>
                    )}
          ...
          {logs.length === 0 && (
            <div className="py-16 text-center font-body-md text-body-md text-on-surface-variant">{t('audit.no_match')}</div>
          )}
        </div>
      </div>
    </div>
